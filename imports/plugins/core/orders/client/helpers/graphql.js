...
export function getOrderRiskStatus(order) {
  const isPaymentNormalRisk = order.payments.every((payment) => {
    console.log("payment", payment);

    return !payment.riskLevel || payment.riskLevel === "normal";
  });
  // const isPaymentNormalRisk = order.payments.every((payment) => !payment.riskLevel || payment.riskLevel === "normal");

  console.log(" --------------- isPaymentNormalRisk", isPaymentNormalRisk);

  // normal transactions do not need to be flagged
  if (isPaymentNormalRisk) {
    return "";
  }

  return "One or more payments have an elevated risk.";
}
