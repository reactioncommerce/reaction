...
test("expect true when a single product variant is sold out and has inventory policy disabled", async () => {
  mockGetVariantQuantity.mockReturnValueOnce(Promise.resolve(0));
  const spec = await isBackorder([mockVariantWithBackorder]);
  expect(spec).toBe(true);
});

test("expect true when an array of product variants are sold out and have inventory policy disabled", async () => {
...
test("expect false when an array of product variants has one sold out and another not sold out and both have inventory policy disabled", async () => {
...
test("expect false when a single product variant is not sold out and has inventory policy disabled", async () => {
...
test("expect false when an array of product variants are not sold out and have inventory policy disabled", async () => {
...
test("expect false when a single product variant is sold out and has inventory policy enabled", async () => {
  mockGetVariantQuantity.mockReturnValueOnce(Promise.resolve(0));
  const spec = await isBackorder([mockVariantWithOutBackorder]);
  expect(spec).toBe(false);
});

test("expect false when a single product variant is sold out and has inventory controls disabled", async () => {
  mockGetVariantQuantity.mockReturnValueOnce(Promise.resolve(0));
