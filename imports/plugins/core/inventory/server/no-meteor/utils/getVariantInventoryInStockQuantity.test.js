...
const internalCatalogProductIdNoInventory = "888";
...
const mockVariantsNoInventory = [
  {
    _id: internalVariantIds[0],
    ancestors: [internalCatalogProductIdNoInventory],
    barcode: "barcode",
    createdAt,
    height: 0,
    index: 0,
...
test("expect product variant quantity number when passing a single variant", async () => {
  mockGetVariants.mockReturnValueOnce(Promise.resolve([mockVariants[0]]));
  const spec = await getVariantInventoryInStockQuantity(mockVariants[0], mockCollections);
...
test("expect product variant quantity number when passing a array of product variant objects", async () => {
  mockGetVariants.mockReturnValueOnce(Promise.resolve([mockVariants[0]]));
  const spec = await getVariantInventoryInStockQuantity(mockVariants[0], mockCollections, mockVariants);
...
test("expect 0 if all variants have an inventory quantity of 0", async () => {
  mockGetVariants.mockReturnValueOnce(Promise.resolve([mockVariantsNoInventory[0]]));
  const spec = await getVariantInventoryInStockQuantity(mockVariantsNoInventory[0], mockCollections, mockVariantsNoInventory);
...
test("expect product variant quantity number when passing a array of product variant objects", async () => {
  mockGetVariants.mockReturnValueOnce(Promise.resolve([mockVariants[0]]));
  const spec = await getVariantInventoryInStockQuantity(mockVariants[0], mockCollections, mockVariantsWithUndefinedInventory);
  expect(spec).toEqual(10);
});
