...
test("publishProductToCatalog ignores deleted variants", async () => {
  const catalogProduct = getCatalogProduct();

  catalogProduct.variants.push(Factory.CatalogVariantSchema.makeOne({
    isDeleted: true,
    isVisible: true,
    variantId: "TOP_VARIANT_2",
    options: null
  }));
  ...
  mockContext.queries.inventoryForProductConfigurations.mockReturnValueOnce(Promise.resolve([
    ...
    },
    {
      productConfiguration: {
        productId,
        productVariantId: "TOP_VARIANT_2"
      },
      inventoryInfo: {
        canBackorder: false,
  ...
  expect(catalogProduct.variants[0].options[0].isSoldOut).toBe(true);
  expect(catalogProduct.variants[0].options[1].isSoldOut).toBe(true);
  expect(catalogProduct.variants[1].isSoldOut).toBe(false);
});
