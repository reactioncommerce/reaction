...
function createShippoParcel(reactionParcel, mass_unit = "kg", distance_unit = "cm") {
  const shippoParcel = {
    "width": reactionParcel.width || "",
    "length": reactionParcel.length || "",
    "height": reactionParcel.height || "",
    "weight": reactionParcel.weight || "",
    distance_unit, // Propably we need to have for each shop a uom/baseuom for distance
    mass_unit // This should be fetched from
  };
  return shippoParcel;
}
...
// Creates a unique Shippo Provider in Shipping Collection for the current Shop
...
function removeShippoShippingProvider() {
  const shippoProvider = Shipping.findOne({
    "shopId": Reaction.getShopId(),
    "provider.name": "Shippo"
  });
  Shipping.remove(shippoProvider._id);
}
...
Meteor.methods({
  "shippo/updateApiKey"(modifier, _id) {
    ...
    const shopId = Packages.findOne({ _id }, { fields: { shopId: 1 } });
  ...
  "shippo/getShippingMethodsForCart"(cartId) {
    ...
    if (cart && cart.userId === this.userId) { // confirm user has the right
      ...
      if (cart.items && cart.items && cart.items[0] && cart.items.parcel) {
