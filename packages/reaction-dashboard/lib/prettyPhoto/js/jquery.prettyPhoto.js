  ...
  e.fn.prettyPhoto = function (s) {
    function g() {
      ...
      $pp_pic_holder.animate({top: projectedTop, left: v / 2 - a["containerWidth"] / 2 < 0 ? 0 : v / 2 - a["containerWidth"] / 2, width: a["containerWidth"]}, settings.animation_speed, function () {
        ...
        if (isSet && S(pp_images[set_position]) == "image") {$pp_pic_holder.find(".pp_hoverContainer").show()} else {$pp_pic_holder.find(".pp_hoverContainer").hide()}
    ...
    e.prettyPhoto.initialize = function () {
      ...
      if (settings.allow_resize) {
        e(window).bind("scroll.prettyphoto", function () {x()});
      }
      e.prettyPhoto.open();
      return false
    };
    ...
    if (!pp_alreadyInitialized && t()) {
      ...
      setTimeout(function () {e("a[" + s.hook + "^='" + hashRel + "']:eq(" + hashIndex + ")").trigger("click")}, 50)
    }
