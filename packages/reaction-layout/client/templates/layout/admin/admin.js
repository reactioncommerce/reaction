...
Template.coreAdminLayout.helpers({
  shortcutButtons() {
    ...
    if (_.isArray(shortcuts)) {
      for (let shortcut of shortcuts) {
        items.push({
          ...
          icon: shortcut.icon,
          tooltip: i18n.t(shortcut.i18nKeyLabel, shortcut.i18n),
    ...
    items.push({
      icon: "plus",
      tooltip: i18n.t("app.createContent", "Create Content"),
      tooltipPosition: "left middle",
      onClick(event) {
    ...
    items.push({
      icon: "edit",
      tooltip: i18n.t("app.toggleEditMode", "Toggle Edit Mode"),
      tooltipPosition: "left middle",
      onClick() {
  ...
  //     //active
  //     icon: data.icon,
  //     tooltip: i18n.t(data.i18nKeyLabel, data.i18n),
  //     tooltipPosition: "left middle"
  //   }
  // },
      ...
      for (let item of registryItems) {
        if (ReactionCore.hasPermission(item.route, Meteor.userId())) {
          ...
          buttons.push({
            href: item.route,
            icon: icon,
            tooltip: i18n.t(item.i18nKeyLabel, item.i18n),
            tooltipPosition: "left middle",
            onClick() {
