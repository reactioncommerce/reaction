...
test("shouldn't apply for 2nd promotion when cart has a promotion applied with stackAbility is none", async () => {
  const cart = {
    _id: "cartId",
    appliedPromotions: [{ ...testPromotion, _id: "test id 2", stackAbility: "none" }]
  };
  mockContext.collections.Promotions = {
    find: () => ({ toArray: jest.fn().mockReturnValueOnce(Promise.resolve([testPromotion])) })
  };
  ...
  expect(mockContext.mutations.saveCart).toHaveBeenCalledWith(mockContext, cart, "promotions");
});

test("shouldn't apply for 2nd promotion when promotion stackAbility is none", async () => {
  const cart = {
    _id: "cartId",
    appliedPromotions: [{ ...testPromotion, _id: "test id 2", stackAbility: "all" }]
  };
  mockContext.collections.Promotions = {
    find: () => ({ toArray: jest.fn().mockReturnValueOnce(Promise.resolve([{ ...testPromotion, stackAbility: "all" }, testPromotion])) })
  };
