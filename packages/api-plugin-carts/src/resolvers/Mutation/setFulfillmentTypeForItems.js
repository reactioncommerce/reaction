 ...
 * @param {String} [args.input.items] - An array of cart items to be assigned to the fulfillment type.
...
export default async function setFulfillmentTypeForItems(parentResult, { input }, context) {
  const { cartId: opaqueCartId, clientMutationId = null, fulfillmentType, items: itemsInput, cartToken } = input;
  const cartId = decodeCartOpaqueId(opaqueCartId);
  const items = itemsInput.map((item) => (decodeCartItemOpaqueId(item)));
