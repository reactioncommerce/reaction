 ...
 */
export default async function filterSearchOrders(_, args, context, info) {
  const {
    shopId,
    filter1level,
    filter2level,
  ...
  const query = await context.queries.filterSearchOrders(context, filter1level, filter2level, filter3level, level, shopId);
