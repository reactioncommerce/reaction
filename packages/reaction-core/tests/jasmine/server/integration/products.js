...
describe("core product methods", function () {
  describe("products/cloneVariant", function () {
    ...
    it(
      "number of `child variants` between source and cloned `variants` " + "should be equal", done => {
        let product;
        product = Factory.create("product");
        spyOn(ReactionCore, "hasPermission").and.returnValue(true);
        Meteor.call("products/cloneVariant", product._id, product.variants[
    ...
    it(
      "`variant` and `child variants` media should be intherited when " + "cloning", done => {
        return done();
      });
  });
  ...
  describe("deleteProduct", function () {
    ...
    it("should delete product by admin", function (done) {
      ...
      expect(Meteor.call("products/deleteProduct", product._id)).toBe(1);
