Template.tags.helpers
  tags: ->
    if @_id
      # if this query doesn't return results then we are not on a tag page
      unless Tags.findOne(@_id) and @tagIds isnt @tagIds?
      ...
      if @relatedTagIds
        ...
        relatedTagIds = Tags.find(relatedTagIds: @_id).fetch()
        for tag in relatedTagIds
          tags.push(Tags.findOne(tag._id))
      if Tags.findOne(@_id) then tags.push(Tags.findOne(@_id))
      tags
  ...
  activeTag: (currentTagId) ->
    if @_id is currentTagId then return true
