 ...
 */
class FileCollection extends _events.EventEmitter {
  /**
  ...
  constructor(name, options) {
    ...
    if (isNode && (!(0, _isArray.default)(stores) || stores.length === 0)) {
  ...
  async insert(fileRecord, options = {}) {
    if (!(fileRecord instanceof _FileRecord.default)) throw new Error("Argument to FileCollection.insert must be a FileRecord instance");
    ...
    return new _FileRecord.default(insertedDoc, { collection: this });
  }
  ...
  async update(id, doc, options = {}) {
    ...
    return new _FileRecord.default(updatedDoc, { collection: this });
  }
  ...
  async remove(id, options = {}) {
    let fileRecord;
    let ID;
    if (id instanceof _FileRecord.default) {
      fileRecord = id;
      ID = fileRecord._id;
    } else {
    ...
    const { stores } = this.options;
    if ((0, _isArray.default)(stores)) {
      await _promise.default.all(stores.map((store) => store.remove(fileRecord)));
    }
  ...
  async findOne(selector, options = {}) {
    ...
    return new _FileRecord.default(doc, { collection: this });
  }
  ...
  async find(selector, options = {}) {
    ...
    return docs.map((doc) => new _FileRecord.default(doc, { collection: this }));
  }
