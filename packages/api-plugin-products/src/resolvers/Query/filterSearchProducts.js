 ...
 */
export default async function filterSearchProducts(_, args, context, info) {
  const {
    shopId,
    filter1level,
    filter2level,
  ...
  const query = await context.queries.filterSearchProducts(context, filter1level, filter2level, filter3level, level, shopId);
