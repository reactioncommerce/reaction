 ...
 */
async function _getDiscountsTotalForCart(context, cart) {
 ...
 * @returns {Object} Object with `discounts` array and `total`
 */
export default async function getDiscountTotalsForCart(context, cart) {
  const orderDiscountTotal = await getCartDiscountTotal(context, cart);
  const itemDiscountTotal = await getItemDiscountTotal(context, cart);
  const shippingDiscountTotal = await getShippingDiscountTotal(context, cart);
  return {
    discounts: cart.discounts,
    total: orderDiscountTotal
  };
}
