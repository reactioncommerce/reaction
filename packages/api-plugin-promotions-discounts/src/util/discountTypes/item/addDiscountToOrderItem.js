...
export default function addDiscountToOrderItem(context, { item, cartItem }) {
  if (typeof item.subtotal === "object") {
    if (!item.subtotal.undiscountedAmount) {
      ...
      const itemTotal = calculateDiscountedItemPrice(context, { price: item.price.amount, quantity: item.quantity, discounts: cartItem.discounts });
      item.subtotal.amount = itemTotal;
    }
  } else {
    item.undiscountedAmount = item.amount;
    const itemTotal = calculateDiscountedItemPrice(context, { price: item.price.amount, quantity: item.quantity, discounts: cartItem.discounts });
