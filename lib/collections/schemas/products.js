...
import { ReactionProduct, getSlug, getDefaultParcelSize } from "/lib/api";
...
export const ProductVariant = new SimpleSchema({
  ...
  weight: {
    ...
    decimal: true,
    autoValue: function () {
      const weight = getDefaultParcelSize("weight");
      if (this.isInsert && weight) {
        return weight;
      } else if (this.isUpsert) {
        return {
          $setOnInsert: weight
  ...
  length: {
    ...
    decimal: true,
    autoValue: function () {
      const length = getDefaultParcelSize("length");
      if (this.isInsert && length) {
        return length;
      } else if (this.isUpsert) {
        return {
          $setOnInsert: length
  ...
  width: {
    ...
    decimal: true,
    autoValue: function () {
      const width = getDefaultParcelSize("width");
      if (this.isInsert && width) {
        return width;
      } else if (this.isUpsert) {
        return {
          $setOnInsert: width
  ...
  height: {
    ...
    decimal: true,
    autoValue: function () {
      const height = getDefaultParcelSize("height");
      if (this.isInsert && height) {
        return height;
      } else if (this.isUpsert) {
        return {
          $setOnInsert: height
