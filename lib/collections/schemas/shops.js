...
export const ParcelSize = new SimpleSchema({
  weight: {
    type: Number,
    defaultValue() {
      const baseUOM = this.siblingField("baseUOM");
      return convertWeight("lb", baseUOM, 8);
    },
    min: 0
  },
  height: {
    type: Number,
    defaultValue() {
      const baseUOL = this.siblingField("baseUOL");
      return convertLength("in", baseUOL, 6);
    },
    min: 0
  },
  length: {
    ...
    defaultValue() {
      const baseUOL = this.siblingField("baseUOL");
      return convertLength("in", baseUOL, 11.25);
    },
    min: 0
  },
  width: {
    ...
    defaultValue() {
      const baseUOL = this.siblingField("baseUOL");
      return convertLength("in", baseUOL, 8.75);
    },
    min: 0
  }
}, { check, tracker: Tracker });
