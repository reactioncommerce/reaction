...
const imgTransforms = [
  { name: "image" },
  { name: "large", transform: { size: 1000, helper: "max" } },
  { name: "medium", transform: { size: 600, helper: "max" } },
  { name: "small", transform: { size: 235, helper: "png" } },
  { name: "thumbnail", transform: { size: 100, helper: "png" } }
];

// building the Grid FS Store
const buildGFS = (img) => new FS.Store.GridFS(img.name, {
  chunkSize: 1 * 1024 * 1024,
  transformWrite(fileObj, readStream, writeStream) {
    if (sharp && img.transform) {
      const { size, helper } = img.transform;
      const transform = sharp().resize(size, size)[helper]();
...
// creating array of image stores
const stores = imgTransforms.map(img => buildGFS(img));

// export the new Media collection
export const Media = new FS.Collection("Media", {
  stores,
  filter: {
    allow: {
