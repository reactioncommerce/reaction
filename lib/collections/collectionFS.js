 ...
 * @property {string} helper - transform helper function call,
 ...
 * so we use the `max` Sharp helper function.
 ...
 * {@link http://sharp.pixelplumbing.com/en/stable/api-output/#png|Sharp png()}
 ...
 */
const imgTransforms = [
  { name: "image" }, // creates the first `image` GridFS store
  { name: "large", transform: { size: 1000, helper: "max" } }, // max helper
  { name: "medium", transform: { size: 600, helper: "max" } }, // max helper
  { name: "small", transform: { size: 235, helper: "png" } }, // png helper
  { name: "thumbnail", transform: { size: 100, helper: "png" } } // png helper
];
 ...
 */
const buildGFS = (img) => new FS.Store.GridFS(img.name, {
  chunkSize: 1 * 1024 * 1024,
  transformWrite(fileObj, readStream, writeStream) {
    if (sharp && img.transform) { // if sharp is loaded and the img has a transform
      const { size, helper } = img.transform;
      const transform = sharp().resize(size, size)[helper](); // resizing image and adding helper
