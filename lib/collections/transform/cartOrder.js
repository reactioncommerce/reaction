...
export const cartOrderTransform = {
  ...
  /**
   * @summary Surcharges for cart/order.
   * @description Grabs surcharges from the invoice records if they exist, otherwise from this.surcharges
   * @example const cartSurcharges = cart.getSurcharges();
   * @method getSurcharges
   * @return {Number} Total value of surcharges
   */
  getSurcharges() {
    let cartSurcharges = 0;
    cartSurcharges = (this.surcharges || []).reduce((acc, item) => {
      if (item && item.amount) {
        return acc + item.amount.amount;
      }
      return acc;
    }, 0);
    const surcharges = cartSurcharges || 0;
    return accounting.toFixed(surcharges, 2);
  },
  /**
  ...
  getTotal() {
    ...
    const surcharges = parseFloat(this.getSurcharges());
    const total = discountTotal + shipping + taxes + surcharges;
  ...
  getUniquePaymentMethods() {
    const uniqueMethods = {};
    for (const group of this.shipping) {
      const { payment } = group;
