 ...
 * Usethese methods on Cart and Orders in templates, `{{cart.getCount}}` and in code, `Cart.findOne().getTotal()`.
 * These use Mongo Collection {@link http://docs.meteor.com/api/collections.html#Mongo-Collection transforms}.
...
export const cartOrderTransform = {
   ...
   * @return {[type]} Object with a key for each shopId in cart/order where the value is the tax total for that shop
   */
  getTaxesByShop() {
