...
/**
 * @method getDefaultParcelSize
 * @summary get shops default parcel size
 * @param {String} prop - prop to be returned
 * @since 1.5.5
 * @return {Number} - weight, length, width, or height
 */
export function getDefaultParcelSize(prop) {
  const shopId = getShopId();
  if (shopId) {
    const shop = Shops.findOne({
      _id: shopId
    ...
    if (shop && shop.defaultParcelSize) {
      return shop.defaultParcelSize[prop];
    }
  }
}
