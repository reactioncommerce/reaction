...
export default {
  ...
  /**
   * @method getVariantAvailableToSellQuantity
   * @memberof Catalog
   * @description calculate a sum of descendants `inventoryAvailableToSell`
   * @param {Object} variant - top-level variant
   * @return {Number} summary of options quantity
   */
  getVariantAvailableToSellQuantity(variant) {
    const options = this.getVariants(variant._id);
    if (options && options.length) {
      return options.reduce((sum, option) => sum + parseInt(option.inventoryAvailableToSell, 10) || 0, 0);
    }
