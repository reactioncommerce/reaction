...
ReactionProduct.setProduct = (currentProductId, currentVariantId) => {
  ...
  const product = Products.findOne({
    $or: [
      ...
      { previousHandle: productId } // Last attempt: the permalink may have changed.
    ]
  });
  ...
  if (product) {
    if (Router.getParam("handle") !== product.handle && product.previousHandle && product.previousHandle !== product.handle) {
