...
Rates service to get automatic conversion for a user's locale. Add the entry to `Shops.json` in the 
`reaction-sample-data` package to load the currency on install/reset.
