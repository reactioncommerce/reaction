...
test("include isDeleted", async () => {
  ...
  expect(mockContext.userHasPermissionLegacy).toHaveBeenCalledWith(["admin", "owner", "tags"], null, { shopId: mockShopId });
...
test("include not visible - by an admin", async () => {
  ...
  expect(mockContext.userHasPermissionLegacy).toHaveBeenCalledWith(["admin", "owner", "tags"], null, { shopId: mockContext.shopId });
  expect(mockContext.collections.Tags.find).toHaveBeenCalledWith({ shopId: mockContext.shopId, isDeleted: false, isVisible: true });
  expect(result).toBe("CURSOR");
});
