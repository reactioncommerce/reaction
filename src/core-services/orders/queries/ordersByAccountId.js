...
export default async function ordersByAccountId(context, { accountId, orderStatus, shopIds } = {}) {
  ...
  for (const shopId of shopIds) {
    await context.validatePermissions("reaction:legacy:orders", "read", { shopId, owner: accountId, legacyRoles: ["orders", "order/fulfillment"] }); // eslint-disable-line no-await-in-loop
  }
